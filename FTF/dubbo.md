...
#### 简述Minor GC 、Major GC 和 Full GC

Minor GC ：新生代的垃圾回收，对象存活率低，通常采用“复制”算法进行回收。通常速度较快。

Major GC：针对老年代的垃圾回收，对象存活率较高，可采用“标记-清除”或“标记-整理”算法。速度相对较慢。

Full GC：针对全局的垃圾回收，三者往往都会导致STW，但STW时间不同，Full GC > Major GC > Minor GC；
...
1. 设置堆大小为稳定值，即－Xms与－Xmx设置为一样的，这样设置会减少GC次数。
2. 设置合理的新生代比例(Survivor和Eden)，默认的只是统计学数字，未必和自己的系统适应。
3. 设置对象进入老年代的年龄。－XX:MaxTenuringThreshold来设置，它默认是15。
